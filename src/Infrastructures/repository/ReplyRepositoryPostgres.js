...
class ReplyRepositoryPostgres extends ReplyRepository {
  ...
  async getReplyByCommentId(commentId) {
    const query = {
      text: 'SELECT r.id, comment_id as "commentId", u.username, r.created_at as date, r.content FROM thread_comment_replies as r JOIN thread_comments as c ON c.id = r.comment_id JOIN users u ON u.id = r.user_id WHERE comment_id = ANY($1::varchar[]) ORDER BY r.created_at ASC',
      values: [commentId],
    };
    return this.pool.query(query);
  }
